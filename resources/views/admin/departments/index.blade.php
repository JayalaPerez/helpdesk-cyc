    ...
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
      ...
      <div class="bg-white shadow-sm sm:rounded-xl p-4">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold">Listado de departamentos</h3>
        ...
        {{-- Crear --}}
        <form method="POST" action="{{ route('admin.departments.store') }}" class="mb-4 flex gap-2 flex-wrap">
          @csrf
          <input name="name" class="border rounded-lg px-3 py-2" placeholder="Nombre del departamento" required>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm border border-zinc-200 sm:rounded-xl overflow-hidden">
            ...
            <tbody class="divide-y divide-zinc-200 bg-white">
                ...
                <tr>
                  ...
                  <td class="px-3 py-2">
                    <div class="flex gap-2">
                      {{-- Editar inline --}}
                      <form method="POST" action="{{ route('admin.departments.update', $d) }}" class="flex gap-2 items-center">
                        @csrf
                        @method('PUT')
                        <input name="name" value="{{ $d->name }}" class="border rounded px-2 py-1 text-sm">
                        <label class="inline-flex items-center gap-1 text-xs text-zinc-600">
                          <input type="checkbox" name="is_active" value="1" @checked($d->is_active)>
                          Activo
                        </label>
                      ...
                      <form method="POST" action="{{ route('admin.departments.destroy', $d) }}"
              ...
              @empty
                <tr>
                  <td colspan="3" class="px-3 py-4 text-center text-zinc-500">Sin departamentos.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
